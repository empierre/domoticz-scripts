...
my $base="/home/pi/";
my $conf="/home/pi/.conf-mysensors";
my $ccnt;
my $cfg;
...
open(FIC,'>/tmp/mysensors-gw.pid')||warn $!;
print FIC $$;
close(FIC);

# SQLite database connexion
if (! -e "/home/pi/mysensors.db") {
	$dbh=&init_database();
	&create_table($dbh);
...
while(1) {
       ...
       foreach (@vals) {
			...
			#Answer the node ID
                        my $msg = "$radioId;$childId;3;0;4;20\n";
                        my $co = $ob->write($msg);
                        warn "write failed\n" unless ($co);
                        print "$date W ($co) : $msg \n";
			...
			`curl -s "http://$domo_ip:$domo_port/json.htm?type=command&param=udevice&idx=243&nvalue=$val&svalue=0;$h" &`;
		}
 		...
 		if (($messageType==1)&&($subType>=40)) {
			...
			$sensor_tab{$radioId}->{$subType}=$payload;
			&update_or_insert($radioId,$subType,$payload);
			if (($radioId==4)&&($subType==43)) {
					print "sending $payload to DZ 425 $payload\n";
					`curl -s "http://$domo_ip:$domo_port/json.htm?type=command&param=udevice&idx=425&nvalue=$payload" &`;
			}
			if (($radioId==8)||($radioId==18)) {
				if ($subType==40) {#Smoke
					...
					if ($childId==0) { 
						$payload*=1000;
						print "sending $payload to DZ 427 $payload\n";
						`curl -s "http://$domo_ip:$domo_port/json.htm?type=command&param=udevice&idx=427&nvalue=$payload" &`;
					} elsif ($childId==1) {
						$payload*=1000;
						...
						$payload*=1000;
						print "sending $payload to DZ 428 $payload\n";
						`curl -s "http://$domo_ip:$domo_port/json.htm?type=command&param=udevice&idx=428&nvalue=$payload" &`;
					}
				} elsif ($subType==44) {#CO
					print "sending $payload to DZ 209 $payload\n";
					`curl -s "http://$domo_ip:$domo_port/json.htm?type=command&param=udevice&idx=209&nvalue=$payload" &`;
 		...
 		if (($messageType==1)&&($subType==11)) {
			# save a UV
			$sensor_tab{$radioId}->{$subType}=$payload;
			&update_or_insert($radioId,$subType,$payload);
			print "sending to DZ 423 $payload\n";
			`curl -s "http://$domo_ip:$domo_port/json.htm?type=command&param=udevice&idx=423&svalue=$payload" &`;
		}
 		if (($messageType==1)&&($subType==23)) {
			# save a Lux
			$sensor_tab{$radioId}->{$subType}=$payload;
			&update_or_insert($radioId,$subType,$payload);
			print "sending to DZ 361 $payload\n";
			`curl -s "http://$domo_ip:$domo_port/json.htm?type=command&param=udevice&idx=361&svalue=$payload" &`;
		}
 		if (($messageType==1)&&($subType==38)) {
			# save a Voltage
			$sensor_tab{$radioId}->{$subType}=$payload;
			if ($childId==10) {
				$TSurl="http://api.thingspeak.com/update?key=BCZ675JM3V43QYBA";
				$TSurl.="&field1=$payload";	
			} elsif ($childId==11) {
				$TSurl.="&field3=$payload";	
			}
			#print "sending to TS $payload $TSurl\n";
			#`curl -s $TSurl &`;
		}
 		if (($messageType==1)&&($subType==39)) {
			# save a Current
			$sensor_tab{$radioId}->{$subType}=$payload;
			#my $TSurl="http://api.thingspeak.com/update?key=BCZ675JM3V43QYBA";
			if ($childId==10) {
				$TSurl.="&field2=$payload";	
			} elsif ($childId==11) {
				$TSurl.="&field4=$payload";	
				print "sending to TS $payload $TSurl\n";
				`curl -s "$TSurl" &`;
			}
		}
